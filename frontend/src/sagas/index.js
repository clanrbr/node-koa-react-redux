...
function* getAllTodos () {
  try {
    const res = yield call(fetch, 'v1/todo')
...
function* saveTodo (action) {
  try {
    ...
    const res = yield call(fetch, 'v1/todo', options)
...
function* deleteTodo (action) {
  try {
    yield call(fetch, `v1/todo/${action.id}`, { method: 'DELETE' })
  } catch (e) {
    yield put(todosFailure(e.message))
  }
}
...
function* updateTodo (action) {
  try {
    yield call(fetch, `v1/todo/${action.id}`, { method: 'POST' })
  } catch (e) {
    yield put(todosFailure(e.message))
  }
}
